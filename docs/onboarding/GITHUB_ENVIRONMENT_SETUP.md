...
|---------|----------|--------|
| **全体所要時間** | **1.5-2.5時間** | **★★☆** |
| 第1章: WSL2のインストール（Windows） | 15-30分 | ★★☆ |
...
graph TB
    Start([開始]) --> OS{OSの確認}
    OS -->|Windows| WSL[第1章<br/>WSL2インストール]
    OS -->|Mac/Linux| Git[第2章<br/>Gitインストール]
    WSL --> Git
...
| 順序 | 作業項目 | 前提条件 | 必須/任意 |
|------|----------|----------|-----------|
| 1 | 第1章: WSL2インストール | Windows環境 | Windows必須 |
| 2 | 第2章: Gitインストール | WSL2完了（Windows）<br/>なし（Mac/Linux） | 必須 |
...
**その他の主要ディストリビューション：**
- **Debian**: Ubuntuの基礎となるディストリビューション（より軽量）
- **Alpine**: 軽量コンテナ向け（開発環境には不向き）
- **Kali Linux**: セキュリティテスト用（特殊用途）
- **openSUSE**: 企業向けLinux（Ubuntuより複雑）

> ⚠️ **注意**: 特別な理由がない限り、**Ubuntu-24.04 LTS**の使用を強く推奨します。
...
| **所要時間** | 10-15分 |
| **難易度** | ★☆☆ |
| **前提条件** | WSL2完了（Windows）またはターミナアクセス（Mac/Linux） |
